...
public class WorkerController {
    ...
    @Operation(summary = "QR 체크인",
            description = "근무 시작 시 QR 스캔으로 체크인합니다. 근무지 반경 100m 이내에서만 가능합니다. " +
                    "정상 체크인 시 노쇼 체크 예약이 자동 취소됩니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "체크인 성공"),
            @ApiResponse(responseCode = "400", description = "이미 체크인됨 / 매칭 상태 부적합 / 근무지 반경 벗어남", content = @Content),
    ...
    public ResponseEntity<Void> checkIn(
        ...
        checkInUseCase.checkIn(
                new CheckInUseCase.Command(matchId, userId, request.latitude(), request.longitude())
        );
        return ResponseEntity.ok().build();
    }

    @Operation(summary = "QR 체크아웃",
            description = "근무 종료 시 QR 스캔으로 체크아웃합니다. 근무지 반경 100m 이내에서만 가능합니다. " +
                    "체크아웃 시 근무 시간 기반으로 정산이 자동 생성되고 매칭 상태가 COMPLETED로 변경됩니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "체크아웃 성공"),
            @ApiResponse(responseCode = "400", description = "체크인 안 됨 / 이미 체크아웃됨 / 근무지 반경 벗어남", content = @Content),
            @ApiResponse(responseCode = "403", description = "본인의 매칭이 아님", content = @Content),
            @ApiResponse(responseCode = "404", description = "존재하지 않는 매칭", content = @Content)
    })
    ...
    public ResponseEntity<Void> checkOut(
        ...
        checkOutUseCase.checkOut(
                new CheckOutUseCase.Command(matchId, userId, request.latitude(), request.longitude())
        );
        return ResponseEntity.ok().build();
    }
}
